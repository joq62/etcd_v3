...
start()->
    ...
    EtdcStart=[{Node,rpc:call(Node,etcd,appl_start,[[]])}||Node<-Nodes],
    io:format("DBG EtdcStart ~p~n",[{EtdcStart,?MODULE,?FUNCTION_NAME,?LINE}]),
    
    io:format("1. InitialNode mnesia:System_info ~p~n",[{InitialNode,rpc:call(InitialNode ,mnesia,system_info,[]),?MODULE,?FUNCTION_NAME,?LINE}]),
    
    
    io:format("INIT STOP ************ ~p~n",[{rpc:call(node() ,init,stop,[]),?MODULE,?FUNCTION_NAME,?LINE}]),
    timer:sleep(2000),
